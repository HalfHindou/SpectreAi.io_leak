/**
 * Company news proxy (Express dev) — Google News RSS, English.
 * GET /api/company-news?q=SpaceX → [{ headline, url, source, date, summary }].
 * Mirrors apps/research/api/company-news.js. Never throws → [].
 */
...
async function fetchCompanyNews(query, limit = 20) {
  const q = String(query || '').trim();
  if (!q) return [];
  const ck = `gnews:${q.toLowerCase()}`;
  const cached = getCached(ck, 5 * 60000);
