 ...
 */
const { chat: gatewayChat } = require('../llm-gateway')
...
async function llmExtract(texts) {
  ...
  try {
    // Mechanical name extraction → 'fast' tier; JSON object out. Routed through
    // the resilient gateway (multi-provider failover) so the backfill keeps
    // working through a single-provider outage. Degrades to [] on total failure.
...
module.exports = { getNameMentions, getRecentTweets, llmExtract, resolveName, PROVIDER }
