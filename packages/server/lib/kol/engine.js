/**
 * KOL Radar — snapshot/diff + convergence signal engine.
 *
...
async function runSync(opts = {}) {
  ...
  try {
    // opts.refreshRegistry = rebuild the KOL DB itself first (upstream seed +
    // curated merge + direct X resolves), not just the follow snapshots.
    const kols = opts.refreshRegistry
      ? await registry.refresh({ force: true })
      : await registry.getKols()
