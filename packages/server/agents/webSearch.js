 ...
 *
 * CommonJS twin of apps/research/api/_lib/websearch.js. The two files are the
 * same ladder for the two runtimes (Vercel ESM / Express CJS); keep them in
 * step — a fix applied to one and not the other is how monarch-chat ended up
 * with two different macro lanes.
 *
 * WHY THIS EXISTS
...
async function searchForContext(query, opts = {}) {
  const { results } = await webSearch(query, opts)
  return results.length ? results : null
}

module.exports = { webSearch, searchForContext, searchSpectre, searchTavily, searchBrave, searchSerp };
