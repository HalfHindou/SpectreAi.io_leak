/**
 * Spectre Data helper — triple-tier market-data lookup.
 *
 * Originally shipped to research-app for Lever 3 (PR #713). Ported verbatim
 * to trading-app for L4-PR1 (2026-06-03) so token-details paths in both
 * apps can route through Hetzner-primary + KV-cached lookups in place of
 * Codex. The trading-app handler previously had NO Hetzner backfill at all.
 *
 * Purpose: stop calling Codex for fields that exist in cheaper sources.
 * Pre-L4 reality: details/details-batch handlers fired filterTokens with a
 * heavy selection (volume24, liquidity, marketCap, change4, change12,
 * holders, txnCount24 each trigger a billable
 * listPairsWithMetadataForToken lockstep op). L4 inverts the routing: this
 * helper is now the PRIMARY source and Codex is the bottom-tier fallback.
 *
