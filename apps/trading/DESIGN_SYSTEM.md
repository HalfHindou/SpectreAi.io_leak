...
## Apple-Style Search Bar UI/UX Rules
## Trading Terminal Application

This document defines **mandatory UI/UX rules** for implementing the search bar in the trading terminal.
The goal is to achieve **Apple Human Interface–level quality**: calm, premium, precise, and invisible-by-default.

These rules are **non-negotiable**.

### Design Philosophy

Search is a **utility**, not a feature.

- It must feel neutral, calm, and secondary
- It must never visually compete with trading data
- If the user is not interacting with it, it should almost disappear
- The design must feel native to macOS / iOS, not “crypto UI”

If the search bar feels “cool”, “flashy”, or “decorative” — it is wrong.

### Visual Hierarchy

#### Idle State (Default)

- Blends into surrounding glass surface
- No accent colors
- Border barely visible
- Slightly lighter than surrounding background
- No glow, no gradient, no animation

#### Hover State

- Slight lift using shadow only
- Very small increase in border opacity
- No color shift

#### Focus State

- Still neutral and calm
- Slight brightness increase
- Subtle white-opacity focus ring
- Accent colors are **NOT allowed** on the container

Accent color usage is allowed **only** for:
- Text caret
- Highlighted search results

### Shape & Sizing

- Height: taller than standard inputs (Apple spacing)
- Border radius: pill or near-pill
- No sharp corners
- Must feel “cut into glass”, not floating above it

### Background Treatment

- Background must be **stable and neutral**
- Must NOT inherit token accent gradients
- Must NOT animate with token color changes
- Uses subtle dark glass only

Allowed techniques:
- Low-opacity white overlay
- Soft backdrop blur
- Inner highlight

The search bar must remain visually consistent even when the token background changes.

### Typography

- Font: `var(--font-body)`
- Font size: calm, readable (≈14px)
- Placeholder text:
  - Lower contrast than body text
  - Descriptive, not promotional

Example placeholder tone:
> “Search tokens or pairs”

### Icon Rules

- SF Symbols–style magnifier icon
- Muted opacity
- No color
- No hover animation
- Icon must not draw attention

If an icon feels “crypto-styled” → remove it.
...
- Surrounding UI may dim slightly (≈0.85 opacity)
- Background visual noise should be reduced
- Results appear with opacity fade-in, not slide-in
- Interaction context should feel calm and intentional

This should feel similar to macOS Spotlight behavior.
...
- Same border radius
- Same glass material
- Appears visually connected to the search bar (same surface language, calm fade-in)
