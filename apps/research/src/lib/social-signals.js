...
export function computeFadeSignal(input = {}) {
  ...
  // ORGANIC vs MANUFACTURED. Unique-voice BREADTH is the dispositive signal: a
  // manufactured/coordinated pump is a handful of accounts x many posts — it
  // cannot have ~1000 distinct authors. A broad crowd (hundreds of unique
  // voices) whose clean signal isn't terrible and isn't quarantined is organic;
  // its high cashtag-only / promo % is an excited memecoin crowd tweeting
  // "$TICKER 🚀" + giveaways, NOT a shill ring. So don't call a positive-price
  // spike "manufactured hype" at that breadth — let the ignition / bullish-crowd
  // read lead. ($ANSEM was wrongly tagged: 988 unique voices, 71% clean, elite
  // quality — the definition of a real crowd, not manufactured.)
  ...
  // pump is one echo chamber; interest spread across >=3 archetypes, VCs/devs/
  // researchers/traders/media, not just trenchers, is real). Both gated on a
  // non-terrible clean signal + not quarantined so a botnet can't fake it.
