/**
 * SVG icon set for WelcomePage - extracted to module scope to avoid
 * re-creating JSX elements on every render.
 */
...
export const icons = {
  ...
  // Market AI toolbar
  aiAnalysis: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3.75 6A2.25 2.25 0 016 3.75h2.25A2.25 2.25 0 0110.5 6v2.25a2.25 2.25 0 01-2.25 2.25H6A2.25 2.25 0 013.75 8.25V6zM3.75 15.75A2.25 2.25 0 016 13.5h2.25a2.25 2.25 0 012.25 2.25V18a2.25 2.25 0 01-2.25 2.25H6A2.25 2.25 0 013.75 18v-2.25zM13.5 6a2.25 2.25 0 012.25-2.25H18A2.25 2.25 0 0120.25 6v2.25A2.25 2.25 0 0118 10.5h-2.25a2.25 2.25 0 01-2.25-2.25V6zM13.5 15.75a2.25 2.25 0 012.25-2.25H18a2.25 2.25 0 012.25 2.25V18A2.25 2.25 0 0118 20.25h-2.25a2.25 2.25 0 01-2.25-2.25v-2.25z"/></svg>,
  ...
  tabs: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7.5" height="7.5" rx="2"/><rect x="13.5" y="3" width="7.5" height="7.5" rx="2"/><rect x="3" y="13.5" width="7.5" height="7.5" rx="2"/><rect x="13.5" y="13.5" width="7.5" height="7.5" rx="2"/></svg>,
...
export const marketAiTabIcons = {
  brief: icons.ai || '\u2726',
  analysis: icons.aiAnalysis,
  // earnings \u2014 a calendar with the print day marked
  earnings: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3.5" y="4.5" width="17" height="16" rx="2"/><path d="M3.5 9h17M8 3v3M16 3v3"/><circle cx="12" cy="14.5" r="2" fill="currentColor" stroke="none"/></svg>,
  news: icons.news,
  posts: <svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>,
  ...
  // podcasts — a microphone, the one unambiguous mark for spoken audio
  podcasts: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="2.5" width="6" height="11" rx="3"/><path d="M5.5 11a6.5 6.5 0 0 0 13 0M12 17.5V21M9 21h6"/></svg>,
  // the long tail — a scatter of decreasing-size marks = the many small caps
  // outside the top 100 (OTHERS2)
  others: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="6.5" cy="7" r="2.3"/><circle cx="14.5" cy="6" r="1.5"/><circle cx="19" cy="9.5" r="1"/><circle cx="9" cy="14.5" r="1.4"/><circle cx="16" cy="15" r="1"/><circle cx="6.5" cy="18.5" r="0.8"/><circle cx="12" cy="19.5" r="0.7"/></svg>,
}

// Category tab icons — monochrome, currentColor only. The chip's text color
// ramp (muted -> warm-white on active) is the only "color" these ever get.
// No per-category brand hues, no smiley/robot/toy glyphs: minimal geometric
// marks that read finance-grade at 13px.
const _cs = { viewBox: '0 0 24 24', fill: 'none', stroke: 'currentColor', strokeWidth: 1.5, strokeLinecap: 'round', strokeLinejoin: 'round' }

export const categoryIcons = {
  // Rows of a table
  all:        <svg {..._cs}><path d="M4 6h16M4 12h16M4 18h16"/></svg>,
  // Stacked liquidity layers
  defi:       <svg {..._cs}><path d="M12 3L3 7.5l9 4.5 9-4.5L12 3zM3 12.5l9 4.5 9-4.5M3 17l9 4.5 9-4.5"/></svg>,
  // Compute chip
  ai:         <svg {..._cs}><rect x="7.5" y="7.5" width="9" height="9" rx="2"/><path d="M12 7.5V4M12 20v-3.5M7.5 12H4M20 12h-3.5"/></svg>,
  // Attention pulse
  meme:       <svg {..._cs}><path d="M3 12h4l3-7 4 14 3-7h4"/></svg>,
  // Institution columns
  rwa:        <svg {..._cs}><path d="M3 21h18M4 21V9.5L12 4l8 5.5V21M9 21v-7h6v7"/></svg>,
  // Feather (Robinhood mark, monochrome)
  robinhood:  <svg {..._cs}><path d="M20.24 12.24a6 6 0 00-8.49-8.49L5 10.5V19h8.5z"/><path d="M16 8L2 22M17.5 15H9"/></svg>,
  // Die
  gamefi:     <svg {..._cs}><rect x="4" y="4" width="16" height="16" rx="3.5"/><circle cx="9" cy="9" r="0.9" fill="currentColor" stroke="none"/><circle cx="15" cy="9" r="0.9" fill="currentColor" stroke="none"/><circle cx="9" cy="15" r="0.9" fill="currentColor" stroke="none"/><circle cx="15" cy="15" r="0.9" fill="currentColor" stroke="none"/></svg>,
  // Rack units
  infra:      <svg {..._cs}><rect x="4" y="4" width="16" height="7" rx="1"/><rect x="4" y="13" width="16" height="7" rx="1"/><path d="M8 7.5h.01M8 16.5h.01"/></svg>,
  // Offset bars (Solana mark abstraction)
  solanaMeme: <svg {..._cs}><path d="M7 7h13M4 12h13M7 17h13"/></svg>,
  // Shield
  privacy:    <svg {..._cs}><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>,
  // Framed artwork
  nft:        <svg {..._cs}><rect x="3.5" y="3.5" width="17" height="17" rx="2.5"/><circle cx="9" cy="9" r="1.5"/><path d="M20.5 15l-4.5-4.5L6 20.5"/></svg>,
  // Two-way flow
  lending:    <svg {..._cs}><path d="M7.5 21L3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5"/></svg>,
}

export const getActivityIcon = (type) => icons[type] || icons.trending
