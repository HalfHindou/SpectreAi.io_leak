 ...
 * Shows amount selection, asset toggle, and chain context so users
 * understand exactly what they're buying before the payment flow opens.
 */
...
export default function UdDepositPanel({ activeChain, activeNetwork, onContinue, onCancel }) {
  ...
  const handleContinue = async () => {
    ...
    setLoading(true)
    try {
      await onContinue({ amount, asset: selectedAsset })
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="ud-dep-panel">
        ...
        <div className="ud-dep-amounts">
          {QUICK_AMOUNTS.map((amt) => (
            <button
              key={amt}
              type="button"
              className={`ud-dep-amount-pill${!isCustom && selectedAmount === amt ? ' is-active' : ''}`}
      ...
      <div className="ud-dep-section">
        ...
        <div className="ud-dep-asset-toggle">
          <button
            type="button"
            className={`ud-dep-asset-btn${selectedAsset === 'native' ? ' is-active' : ''}`}
          ...
          <button
            type="button"
            className={`ud-dep-asset-btn${selectedAsset === 'usdc' ? ' is-active' : ''}`}
            onClick={() => setSelectedAsset('usdc')}
          >
            <span className="ud-dep-usdc-icon">$</span>
            <span>USDC</span>
          </button>
        </div>
      </div>
      ...
      <div className="ud-dep-summary">
        {finalAmount > 0 && (
          <p className="ud-dep-summary-text">
            Buy ~${finalAmount} of {selectedAsset === 'usdc' ? 'USDC' : nativeSymbol} on {activeNetwork.label}
          </p>
        )}
        <button
          type="button"
          className="ud-dep-continue-btn"
      ...
      <button type="button" className="ud-dep-back" onClick={onCancel}>
        Back to balances
      </button>
    </div>
  )
