/**
 * Vercel Serverless — Spectre Search Engine (SSE streaming).
 *
 * Port of packages/server/routes/search-engine.js. The Express version was
 * the source of truth and works locally; the Vercel `extended-proxy`
 * `handleSearchQuery` was a 6-line stub returning "Search engine requires
 * the Spectre server", which is why /api/search/query was broken in prod.
 *
 * Pipeline:
 *   1. Classify the query (18-way) + detect assets/chain.
 *   2. Plan endpoint fan-out by classification + mode (quick | thesis).
 *   3. Parallel fetch from the Spectre Data API.
 *   4. Stream `data` events as each endpoint resolves (data-first paint).
 *   5. Build a system prompt with the structured data + citation map.
 *   6. Stream LLM tokens via Groq (`llama-3.3-70b-versatile`) with inline
 *      `[n]` citations.
 *   7. Emit `related` (people-also-ask) + `done`.
 *
 * Wired via vercel.json:
 *   /api/search/query  →  /api/data-api?fn=extended-proxy&route=search-query
 * which delegates to this module's default export.
 */
...
function setupSSE(res) {
  ...
  // Vercel Edge/Functions may not have flushHeaders; guard it.
...
const SYMBOL_ALIASES = {
  ...
  aptos: 'APT', apt: 'APT', sui: 'SUI', optimism: 'OP',
  arbitrum: 'ARB', arb: 'ARB', render: 'RNDR', rndr: 'RNDR',
  helium: 'HNT', hnt: 'HNT', bittensor: 'TAO', tao: 'TAO',
  filecoin: 'FIL', fil: 'FIL', pepe: 'PEPE', shib: 'SHIB',
  bonk: 'BONK', wif: 'WIF', ondo: 'ONDO', eigen: 'EIGEN',
  sei: 'SEI', hype: 'HYPE',
}
...
function detectAssets(text) {
  ...
  if (hasLower) {
    ...
    if (capsHits) for (const h of capsHits) if (KNOWN_TICKERS.has(h)) assets.add(h)
  }
...
function detectChain(text) {
  const lc = text.toLowerCase()
  for (const chain of CHAIN_NAMES) if (new RegExp(`\\b${chain}\\b`).test(lc)) return chain
  return null
}
...
function planEndpoints(classification, assets, mode, chain) {
  ...
  const isThesis = mode === 'thesis' || mode === 'deep'
  ...
  const plans = {
    ...
    MARKET_OVERVIEW: [
      ...
      ...(isThesis ? [
        { slot: 'brain', url: '/v1/brain', priority: 2, label: 'brain' },
        { slot: 'news', url: '/v1/news?limit=8', priority: 3, label: 'news' },
        { slot: 'breaking', url: '/v1/news/breaking?limit=5', priority: 3, label: 'breaking' },
      ] : []),
    ],
    DERIVATIVES: [
    ...
    MINDSHARE: [
      ...
      { slot: 'trending', url: '/v1/trending', priority: 1, label: 'trending' },
    ],
  }

  const plan = plans[classification] || plans.MARKET_OVERVIEW
  const maxEndpoints = isThesis ? 14 : 4
  return plan.sort((a, b) => a.priority - b.priority).slice(0, maxEndpoints)
}
...
- $TICKER format for asset symbols ($BTC, $ETH).`
...
export default async function handler(req, res) {
  ...
  res.setHeader('Access-Control-Allow-Origin',
    ['http://localhost:5180', 'http://localhost:5181', 'https://app.spectreai.io', 'https://spectre-app-research.vercel.app'].includes(origin) ? origin : '')
  ...
  const mode = (body.mode || 'quick').toLowerCase()
  const focus = (body.focus || 'all').toLowerCase()

  if (!query) return res.status(400).json({ error: 'query string required' })
  // LLM key is no longer required here — the resilient gateway picks whatever
  // provider is live (Groq → Cerebras/Gemini → OpenRouter → OpenAI/Anthropic →
  // Ollama) and only fails when EVERY provider is down (never-dark).
  if (!SPECTRE_API_KEY) {
    return res.status(503).json({ error: 'Spectre Data API key not configured' })
  }
  ...
  try {
    ...
    // 2. Fan-out + stream data events
    const dataPromises = plan.map(async (call, idx) => {
      const result = await spectreFetch(call.url)
      if (result.ok && result.data != null) {
        send(res, 'data', { slot: call.slot, payload: result.data, endpoint: call.url })
      }
      return {
        id: idx + 1,
        slot: call.slot,
    ...
    const citations = results.map((r) => ({
      id: r.id, endpoint: r.endpoint, label: r.label, latency_ms: r.latency_ms,
    }))
    ...
    const systemPrompt = buildSearchPrompt(
      ...
      mode, results, citations,
    )

    // Stream via the resilient gateway (multi-provider failover + circuit
    // breaker). Same tier/limits as the old direct-Groq call; `smart` = 70B
    // synthesis. If every provider is down the generator yields nothing and we
    // fall back to the handler's existing LLM-error path.
    let fullText = ''
    ...
    try {
      for await (const chunk of chatStream({
        ...
        tier: 'smart',
        maxTokens: (mode === 'thesis' || mode === 'deep') ? 4000 : 1000,
        temperature: (mode === 'thesis' || mode === 'deep') ? 0.6 : 0.4,
        timeoutMs: 45000,
      })) {
        if (aborted) break
        if (chunk) {
      ...
      console.error('[search-engine] LLM stream error:', streamErr?.message)
    }

    if (aborted) return

    // Nothing streamed → all providers failed. Mirror the prior LLM-error path.
    if (!streamedAny) {
      console.error('[search-engine] LLM produced no output (all providers failed)')
    ...
    if (!res.writableEnded) {
      try { send(res, 'error', { message: err?.message || 'internal error' }) } catch {}
      try { res.end() } catch {}
    }
  }
}
