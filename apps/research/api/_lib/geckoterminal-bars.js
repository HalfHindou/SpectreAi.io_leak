...
export const GT_NETWORK_MAP = {
  ...
  999: 'hyperevm',       // Hyperliquid (CG platform slug 'hyperliquid'), slug verified 2026-08-25
  101: 'sui-network',    // Sui, slug verified 2026-08-25
  146: 'sonic',          // Sonic, slug verified 2026-08-25
};
...
async function _findTopPool(network, address) {
  ...
  try {
    ...
    // Which side of the pool is OUR token? `relationships.base_token.data.id`
    // is `<net>_<address>`; net slugs can contain underscores (polygon_pos),
    // so strip via lastIndexOf, never split('_') (charts-system §I13 trap).
    // Consumers (the Codex pair pin) only ever pin a pool whose BASE is the
    // charted token — a quote-side pool prices somebody else's tape.
    let baseAddr = null;
    const baseId = top?.relationships?.base_token?.data?.id;
    if (typeof baseId === 'string') {
      const us = baseId.lastIndexOf('_');
      baseAddr = us >= 0 ? baseId.slice(us + 1) : baseId;
    }
    // Pool birth (free, same response): lets the Codex pair pin skip billed
    // guaranteed-empty getBars for windows that end before the pool existed.
    let createdAtSec = null;
    const createdIso = top?.attributes?.pool_created_at;
    if (createdIso) {
      const ms = Date.parse(createdIso);
      if (Number.isFinite(ms) && ms > 0) createdAtSec = Math.floor(ms / 1000);
    }
    _poolResolutionCache.set(key, { pool: poolAddr, baseAddr, createdAtSec, ts: Date.now() });
    return poolAddr;
  } catch (e) {
...
/**
 * Deterministic pool pin for the Codex bars tier (2026-08-25, ZIG dust-pool
 * follow-up). Codex's `getTokenBars` aggregates "top liquidity pairs" for a
 * token, so a CEX-priced token with dead DEX remnants (ZIG: ~20 Uniswap dust
 * pools, one still trading at 1.45x the real market) gets dust trades blended
 * into — or outright substituted for — its tape, differently per request.
 * Pinning the Codex query to GT's highest-liquidity BASE-side pool makes the
 * pick deterministic and consistent with what the GT tier itself charts.
 *
 * Returns `{ pool, createdAt }` (createdAt = unix seconds of pool birth, or
 * null when GT omitted it) when (a) GT knows a top pool for the token and
 * (b) OUR token is that pool's base side; `null` otherwise (caller falls back
 * to the token-form query — fail-soft, today's behavior).
 */
export async function findTopPoolPin(networkId, address) {
  const gtNetwork = networkIdToGtSlug(networkId);
  if (!gtNetwork || !address) return null;
  const pool = await _findTopPool(gtNetwork, address);
  if (!pool) return null;
  const cached = _poolResolutionCache.get(`${gtNetwork}:${String(address).toLowerCase()}`);
  const baseAddr = cached?.baseAddr;
  // Case-insensitive EQUALITY only (EVM checksums differ in case; Solana
  // base58 is case-sensitive but two distinct mints never differ by case
  // alone in practice, and this compares GT's canonical string to ours).
  if (!baseAddr || String(baseAddr).toLowerCase() !== String(address).toLowerCase()) return null;
  return { pool, createdAt: cached?.createdAtSec || null };
}
