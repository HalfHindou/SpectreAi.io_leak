...
// CG's OHLC candles are SNAPSHOT-derived: a bar's open is not the prior
// close, so consecutive candle RANGES can leave a void nothing "traded" in —
// impossible on a 24/7 asset, and rendered as a torn chart (ZIG 13→14 Jul
// 2021: day 13 low 0.0588 vs day 14 high 0.0542, an 8% void between adjacent
// daily ranges). Stitch ADJACENT bars only: open := prior close, high/low
// widened to include it — the price provably traversed that path on a 24/7
// market, so this ADDS no information CG didn't imply, it removes a sampling
// artifact. Bars across a REAL data hole (> 1.5 buckets — the ZIG 2022
// missing-era class) are left untouched: bridging a hole would fabricate a
// giant candle over an era CG simply lacks.
// Exported: writeBarsPayload (bars-router.js) applies this to EVERY outgoing
// payload as the universal continuity pass — Binance/AMM tapes are already
// continuous (no-op there), snapshot-derived and mixed/hole-filled series get
// their voids closed. Idempotent, so double application (lane + write site)
// is safe. `label`, when given, logs seams the stitch had to move >10% —
// a big closed seam is the "two sources at different levels" signal and
// deserves eyes even though the chart no longer tears.
export function stitchAdjacentOpens(bars, bucketSec, label = null) {
  if (!Array.isArray(bars) || bars.length < 2 || !(bucketSec > 0)) return bars;
  let bigSeams = 0;
  for (let i = 1; i < bars.length; i++) {
    const prev = bars[i - 1];
    const b = bars[i];
    if (b.t - prev.t > bucketSec * 1.5) continue;
    if (!(prev.c > 0) || !(b.o > 0)) continue;
    if (b.o !== prev.c) {
      const r = b.o > prev.c ? b.o / prev.c : prev.c / b.o;
      if (r > 1.1) bigSeams++;
      b.o = prev.c;
      if (b.h < b.o) b.h = b.o;
      if (b.l > b.o) b.l = b.o;
  ...
  if (label && bigSeams > 0) {
    console.warn(`[bars-stitch] ${label}: closed ${bigSeams} seam(s) >10% between adjacent bars — sources may disagree, worth a look`);
  }
  return bars;
}
...
export async function fetchCgOhlcBars(cgId, resolution, fromSec, toSec) {
  ...
  try {
    if (res === '1D' || res === 'D' || res === '1W' || res === 'W' || res === '7D') {
      // Fetch ONE bucket before the requested window: scroll-back pages are
      // separate requests, and without the extra leading bar the FIRST bar of
      // each page keeps its snapshot-derived open — a torn seam at every page
      // boundary. Stitch against the extra bar, then drop it.
      const fromExt = fromSec - DAY;
      const bars = await fetchRange(cgId, 'daily', 179 * DAY, fromExt, toSec, 6);
      let out = stitchAdjacentOpens(dropMigrationArtifacts(dedupeSortFilter(bars, fromExt, toSec), cgId), DAY);
      out = out.filter((b) => b.t >= fromSec);
      if (out.length) {
    ...
    if (res === '60' || res === '240' || res === '720') {
      ...
      // One-bucket lead-in for cross-page seam continuity (see the daily lane).
      const fromExt = cappedFrom - 3600;
      const bars = await fetchRange(cgId, 'hourly', 30 * DAY, fromExt, toSec, 4);
      let out = stitchAdjacentOpens(dropMigrationArtifacts(dedupeSortFilter(bars, fromExt, toSec), cgId), 3600);
      out = out.filter((b) => b.t >= cappedFrom);
      if (res === '240') out = aggregate(out, 4 * 3600);
      if (res === '720') out = aggregate(out, 12 * 3600);
      return out.length ? out : null;
    }
    if (res === '30') {
      // /ohlc serves 30m candles ONLY for days=1 (CG accepts 1/7/14/30/…;
      // days=2 is "Invalid days parameter" — this lane had been dead since it
      // was written, found 2026-08-25 via the HYPE intraday no_data class).
      const j = await cgJson(`/coins/${encodeURIComponent(cgId)}/ohlc?vs_currency=usd&days=1`);
      if (!Array.isArray(j) || j.length === 0) return null;
      // Lead-in bucket for the same cross-page seam reason as the lanes above.
      let out = stitchAdjacentOpens(dedupeSortFilter(j.map(toBar).filter(Boolean), fromSec - 1800, toSec), 1800);
      out = out.filter((b) => b.t >= fromSec);
      return out.length ? out : null;
    }
