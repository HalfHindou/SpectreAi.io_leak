/**
 * KOL Radar — the Giga KOL DB (ESM serverless mirror of the dev CJS registry at
 * packages/server/lib/kol/registry.js).
 *
 ...
 * In-memory cache + persists to the store.
 *
 * Upstream client mirrors the prod X Dash proxy (api/_lib/handlers/social-proxy.js):
 * base from DASHBOARD_API_BASE_URL/X_DASH_BASE (default direct Hetzner origin),
 * auth via XDASH_API_TOKEN/DASHBOARD_API_KEY/X_DASH_API_KEY sent as both
 * `Authorization: Bearer` and `X-API-Key` (the upstream's canonical scheme).
 */
import store from './store.js'
import { getProjectHealth, computeLegitimacy } from './health.js'
...
const AUTH_HEADERS = X_DASH_API_KEY
  ? {
...
// Backfill: the seed carries only 1-12 pushes per KOL (top ENRICH_COUNT
// deep-enriched), so most profiles are thin. On dossier open, pull the KOL's
// FULL token list from /api/author/:id so every profile shows real projects +
// a reliable legitimacy score. Cached + graceful.
async function getRichPushes(kol) {
  if (!kol) return []
  if ((kol.pushes || []).length >= 8) return kol.pushes
