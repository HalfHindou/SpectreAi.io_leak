/**
 * Spectre LLM Gateway (ESM serverless mirror of packages/server/lib/llm-gateway.js) — one resilient, cost-tiered entry point for ALL LLM calls.
 *
...
const PROVIDERS = {
  ...
  anthropic: {
    ...
    // Off the default chain and Haiku on BOTH tiers. If someone opts this back
    // in it is because everything free is down, and an emergency fallback wants
    // to be cheap, not clever — Sonnet 4 is roughly twelve times the price of
    // Haiku 4.5 for a market blurb neither of them finds difficult.
    models: { fast: 'claude-haiku-4-5-20251001', smart: 'claude-haiku-4-5-20251001' },
  },
  ollama: {
...
// expensive"). It stays configured, so `LLM_CHAIN=...,anthropic` opts it back in
// for one deploy or one incident, but nothing reaches it by accident — and
// "by accident" is not hypothetical: it sits at the END of the chain, so the
// only way to reach it is for every free provider to be down at once, which is
// exactly what happened today. A keyed deployment would have quietly billed
// premium tokens for every message of that outage, at the worst possible moment
// to be spending money. The four providers above it are all free at our volume.
...
async function callProvider(p, { messages, tier, maxTokens, temperature, json, timeoutMs }) {
  const cfg = PROVIDERS[p]
  const key = providerKey(p)
  const model = modelFor(p, tier)
  ...
  if (cfg.kind === 'gemini') {
    const { sys, contents } = toGemini(messages)
    const url = `${cfg.base}/models/${model}:generateContent?key=${encodeURIComponent(key)}`
    const body = {
      contents,
      ...(sys ? { systemInstruction: { parts: [{ text: sys }] } } : {}),
      // 🪤 THINKING HEADROOM. On Gemini 2.5+/3 flash, maxOutputTokens is a ceiling
      // on thinking + answer COMBINED, and these models always think. Measured
      // 2026-08-02 asking for 420: thoughtsTokenCount 399, output 17,
      // finishReason MAX_TOKENS - i.e. the JSON came back truncated mid-string
      // and every caller silently fell through to its fallback. Disabling it via
      // thinkingConfig.thinkingBudget = 0 is rejected (400 INVALID_ARGUMENT) on
      // this model, so the only lever is to budget for it. Same request at 2048:
      // thoughts 412, output 80, finishReason STOP, clean JSON. Callers keep
      // asking for the ANSWER length they want; this pays for the reasoning.
      generationConfig: { maxOutputTokens: geminiBudget(maxTokens), temperature, thinkingConfig: { thinkingBudget: GEMINI_THINK_CAP }, ...(json ? { responseMimeType: 'application/json' } : {}) },
    }
    const res = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body), signal: ctrl })
...
async function chat(opts = {}) {
  const { messages, tier = 'smart', maxTokens = 1024, temperature = 0.3, json = false, timeoutMs = 30000, chain: only } = opts
  ...
  for (const p of order) {
    tried.push(p)
    try {
      const r = await callProvider(p, { messages, tier, maxTokens, temperature, json, timeoutMs })
      if (r.ok) return { ok: true, text: r.text, provider: p, model: modelFor(p, tier), tried }
...
export { chat, chatStream, chatRaw, providerStatus, PROVIDERS }
