...
export async function tryGeckoTerminal(ctx) {
  ...
  // src=gt pin already ran (and missed) this tier for this request — the
  // legacy cascade below it must not pay the GT round-trip a second time.
  if (ctx.gtExhausted) return null;
...
export async function tryCgOhlc(ctx) {
  ...
  try {
    // COARSEN, DON'T DIE (2026-08-25, the HYPE class): CG's finest OHLC is
    // 30m, so 1m/5m/15m used to return null here — and for cgId-only tokens
    // (native-chain ids Codex/GT can't index) that killed the whole cascade
    // on the chart's DEFAULT preset ('24h' fetches 5m). Serve the closest
    // supported bucket instead, same rule GT_RESOLUTION_MAP already applies
    // (30→15m, 3D→1d). SERVING tier only — the head-stitch helpers keep
    // calling fetchCgOhlcBars directly so mixed-cadence prepends can't happen.
    const cgRes = ({ '1': '30', '5': '30', '15': '30' })[String(ctx.requestedRes)] || ctx.requestedRes;
    const bars = await fetchCgOhlcBars(ctx.reqCgId, cgRes, ctx.fromSec, ctx.toSec);
    if (bars?.length > 0) {
      ...
      const payload = { bars, source: 'cg-ohlc', meta: { volumeAvailable: bars.some((b) => b.v > 0), ...(cgRes !== ctx.requestedRes ? { effectiveResolution: cgRes } : {}) } };
      try { await setJsonWithTTL(kvKey, payload, ctx.freeTierTtl); } catch { /* best-effort */ }
      return payload;
    }
  } catch (e) {
...
/* ── Off-scale price repair ────────────────────────────────────────────────
   Measured on the codex tier for LEO 5m, 2023-01-12 (rz-chart-audit-plan):

     285: o=4.348      h=4.348  l=3.923      c=3.923
     286: o=3.923      h=3.923  l=4.102e-36  c=4.102e-36   v=107
     287: o=4.102e-36  h=3.029  l=4.102e-36  c=3.029       v=10
     288: o=3.029      h=3.091  l=3.029      c=3.091

   `4.10238194543e-36 * 1e36 = 4.10238194543` - a real LEO price for that window
   (range 2.49..9.10). 1e36 is (1e18)^2, i.e. the token's decimals applied twice
   upstream. It is ONE corrupted price point that legitimately appears in four
   slots, because each bar's open is the prior close and h/l are the body's
   extremes. Reproduced at 5m, 60m AND 1D for the same token/era, so it is in
   Codex's trade records rather than any one aggregation.

   clampOutlierBars cannot see this: it only ever rewrites h and l (a wick
   clamp), and a corrupted CLOSE is out of its scope entirely. Worse, the tiny
   close drags bodyMin to ~0, which makes its low test unsatisfiable and so
   disables the wick guard on that bar too.

   Rendered, the bar is a candle wicking to zero; on a LOG axis it destroys the
   scale. Repair rather than drop, because the magnitude is recoverable and an
   exact power of ten is a unit error, not a market move - but only when the
   correction is decimals-plausible AND lands the value back among its
   neighbours. Anything else is a guess, and the bar is dropped instead. */
const PRICE_FLOOR = 1e-15;              // nothing on our path trades below ~1e-12
const SCALE_MIN_EXP = 6;                // below this it is price action, not units
const SCALE_MAX_EXP = 40;               // two 18-decimal applications plus slack
const SCALE_BAND = 20;                  // the fix must land within 20x of local median
const isPriceValue = (v) => Number.isFinite(v) && v > PRICE_FLOOR;

export function repairScaledPrices(bars, label) {
  if (!Array.isArray(bars) || bars.length < 5) return bars;
  let anyBroken = false;
  for (const b of bars) {
    if (!b || ![b.o, b.h, b.l, b.c].every(isPriceValue)) { anyBroken = true; break; }
  }
  if (!anyBroken) return bars;

  const out = [];
  let repaired = 0, dropped = 0;
  for (let i = 0; i < bars.length; i++) {
    const b = bars[i];
    if (b && [b.o, b.h, b.l, b.c].every(isPriceValue)) { out.push(b); continue; }
    if (!b) { dropped++; continue; }

    // Local median of REAL prices (same +-7 shape clampOutlierBars uses).
    const win = [];
    for (let k = Math.max(0, i - 7); k < Math.min(bars.length, i + 8); k++) {
      const n = bars[k];
      if (n && isPriceValue(n.c)) win.push(n.c);
      if (n && isPriceValue(n.o)) win.push(n.o);
    }
    if (win.length < 3) { out.push(b); continue; }   // nothing to judge against
    win.sort((x, y) => x - y);
    const med = win[Math.floor(win.length / 2)];

    const fix = (v) => {
      if (isPriceValue(v)) return v;
      // A non-positive or non-finite field carries no magnitude to rescale from.
      if (!Number.isFinite(v) || v <= 0) return null;
      const exp = Math.round(Math.log10(med / v));
      if (!(exp >= SCALE_MIN_EXP && exp <= SCALE_MAX_EXP)) return null;
      const scaled = v * Math.pow(10, exp);
      if (!(scaled > med / SCALE_BAND && scaled < med * SCALE_BAND)) return null;
      return scaled;
    };

    const o = fix(b.o), h = fix(b.h), l = fix(b.l), c = fix(b.c);
    if (o === null || h === null || l === null || c === null) { dropped++; continue; }
    // A repaired body can sit outside the old extremes - h/l were derived from
    // the corrupted point. Widen (never narrow) so the bar stays a valid candle:
    // l <= min(o,c) and h >= max(o,c) is the definition of OHLC, not a guess.
    out.push({ ...b, o, h: Math.max(h, o, c), l: Math.min(l, o, c), c });
    repaired++;
  }
  if ((repaired || dropped) && label) {
    console.warn(`[bars-scale] ${label}: repaired ${repaired}, dropped ${dropped} off-scale bar(s)`);
  }
  return out;
}
...
export function writeBarsPayload(res, payload, ctx) {
  ...
  if (payload?.bars?.length > 0) {
    // OFF-SCALE REPAIR runs FIRST (2026-08-26). A price corrupted by a units
    // error poisons everything downstream: sanitizeBars' median band, the wick
    // clamp's bodyMin, and stitchAdjacentOpens (which would stitch the next
    // bar's open to a junk close). See repairScaledPrices above.
    payload = { ...payload, bars: repairScaledPrices(payload.bars, `${ctx?.tokenSymbol || '?'} ${ctx?.requestedRes || '?'}`) };
    ...
    // UNIVERSAL CONTINUITY PASS (2026-08-25, "no more torn charts"): every
    // outgoing series gets adjacent-bar opens stitched to the prior close.
    // Crypto is 24/7 — adjacent candle ranges that don't touch are a sampling
    // artifact (snapshot-built cg-ohlc candles, mixed/hole-filled series,
    // sanitize-flattened bars), never market truth. Binance/AMM tapes are
    // already continuous, so this is a no-op for them; real DATA HOLES
    // (> 1.5 buckets) are never bridged. Runs AFTER sanitizeBars so a
    // flattened glitch bar reconnects to its neighbours. Idempotent.
    const stitchBucket = ctx?.isWeekly ? 604800 : (ctx?.bucketSec || null);
    if (stitchBucket) {
      stitchAdjacentOpens(payload.bars, stitchBucket, `${ctx?.tokenSymbol || '?'} ${ctx?.requestedRes || '?'}`);
    }
  }
